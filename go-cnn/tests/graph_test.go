...
// TestBuildDigitCNN 测试手写数字识别CNN架构
func TestBuildDigitCNN(t *testing.T) {
	// 创建28x28的输入图像 (batch_size=1)
	inputData := matrix.Randn(1, 28*28, 0.0, 0.1)
	input := graph.Input(inputData, "mnist_input")
	...
	t.Logf("卷积层1输出形状: (%d, %d)", conv1.Value.Rows, conv1.Value.Cols)
	t.Log("卷积层1输出:")
	t.Log(conv1.Value.String())

	relu1 := graph.ReLU(conv1)
	t.Logf("ReLU1输出形状: (%d, %d)", relu1.Value.Rows, relu1.Value.Cols)
	t.Log("ReLU1输出:")
	...
	t.Logf("池化层1输出形状: (%d, %d)", pool1.Value.Rows, pool1.Value.Cols)
	t.Log("池化层1输出:")
	t.Log(pool1.Value.String())
	...
	t.Logf("卷积层2输出形状: (%d, %d)", conv2.Value.Rows, conv2.Value.Cols)
	t.Log("卷积层2输出:")
	t.Log(conv2.Value.String())

	relu2 := graph.ReLU(conv2)
	t.Logf("ReLU2输出形状: (%d, %d)", relu2.Value.Rows, relu2.Value.Cols)
	t.Log("ReLU2输出:")
	t.Log(relu2.Value.String())

	// 池化层2: MaxPool2D(pool_size=2x2, stride=2)
	// 输入: 8x8x16 -> 输出: 4x4x16
	t.Log("\n=== 池化层2 ===")
	pool2 := graph.MaxPool2d(relu2, 2, 2, 8, 8, 16)
	t.Logf("池化层2输出形状: (%d, %d)", pool2.Value.Rows, pool2.Value.Cols)
	t.Log("池化层2输出:")
	t.Log(pool2.Value.String())

	// 展平层: Flatten() -> 256个特征 (4*4*16 = 256)
	t.Log("\n=== 展平层 ===")
	flatten := graph.Reshape(pool2, 1, 4*4*16)
	t.Logf("展平层输出形状: (%d, %d)", flatten.Value.Rows, flatten.Value.Cols)
	t.Log("展平层输出:")
	t.Log(flatten.Value.String())

	// 全连接层1: Dense(120) + ReLU
	t.Log("\n=== 全连接层1 ===")
	dense1 := graph.Dense(flatten, 120)
	t.Logf("全连接层1输出形状: (%d, %d)", dense1.Value.Rows, dense1.Value.Cols)
	t.Log("全连接层1输出:")
	t.Log(dense1.Value.String())

	relu3 := graph.ReLU(dense1)
	t.Logf("ReLU3输出形状: (%d, %d)", relu3.Value.Rows, relu3.Value.Cols)
	t.Log("ReLU3输出:")
	t.Log(relu3.Value.String())

	// 全连接层2: Dense(84) + ReLU
	t.Log("\n=== 全连接层2 ===")
	dense2 := graph.Dense(relu3, 84)
	t.Logf("全连接层2输出形状: (%d, %d)", dense2.Value.Rows, dense2.Value.Cols)
	t.Log("全连接层2输出:")
	t.Log(dense2.Value.String())

	relu4 := graph.ReLU(dense2)
	t.Logf("ReLU4输出形状: (%d, %d)", relu4.Value.Rows, relu4.Value.Cols)
	t.Log("ReLU4输出:")
	t.Log(relu4.Value.String())

	// 输出层: Dense(10) (Softmax在损失函数中处理)
	t.Log("\n=== 输出层 ===")
	output := graph.Dense(relu4, 10)
	t.Logf("输出层形状: (%d, %d)", output.Value.Rows, output.Value.Cols)
	t.Log("输出层输出:")
	t.Log(output.Value.String())

	// 检查输出形状应该是 (1, 10) - 10个类别
	if output.Value.Rows != 1 || output.Value.Cols != 10 {
		t.Errorf("CNN输出形状错误: 期望(1,10), 得到(%d,%d)",
			output.Value.Rows, output.Value.Cols)
	}

	// 创建标签并计算损失
	labels := graph.NewConstant(matrix.NewMatrixFromData([]float64{5}, 1, 1), "label")
	loss := graph.SoftmaxCrossEntropyLoss(output, labels, true)
	...
	// 打印各层的梯度
	if output.Gradient != nil {
		t.Log("输出层梯度:")
		t.Log(output.Gradient.String())
	}

	if relu4.Gradient != nil {
		t.Log("ReLU4梯度:")
		t.Log(relu4.Gradient.String())
	}

	if dense2.Gradient != nil {
		t.Log("全连接层2梯度:")
		t.Log(dense2.Gradient.String())
	}
	...
	if relu3.Gradient != nil {
		t.Log("ReLU3梯度:")
		t.Log(relu3.Gradient.String())
	}
	...
	if flatten.Gradient != nil {
		t.Log("展平层梯度:")
		t.Log(flatten.Gradient.String())
	}

	if pool2.Gradient != nil {
		t.Log("池化层2梯度:")
		t.Log(pool2.Gradient.String())
	}

	if relu2.Gradient != nil {
		t.Log("ReLU2梯度:")
		t.Log(relu2.Gradient.String())
	}

	if conv2.Gradient != nil {
		t.Log("卷积层2梯度:")
		t.Log(conv2.Gradient.String())
	}
	...
	if pool1.Gradient != nil {
		t.Log("池化层1梯度:")
		t.Log(pool1.Gradient.String())
	}
	...
	t.Logf("CNN最终输出形状: (%d, %d)", output.Value.Rows, output.Value.Cols)
	t.Logf("最终损失: %f", loss.Value.At(0, 0))
}
